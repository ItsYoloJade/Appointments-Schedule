...
public class Metodos {
	...
	public static void agregarRegistro() {
		Scanner scEspacio = new Scanner (System.in);
		String sepEspacio = "_";
		int sizeRegist = 0;
		 
		try {
			BufferedWriter bf = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(ArchivoTXT, true), "utf-8"));
			System.out.println("Ingrese el nombre completo:");
			NOMBRE = scEspacio.nextLine();
			
			System.out.println("Ingrese el telefono:");
			TELEFONO = scEspacio.nextLine();
			
			System.out.println("Ingrese el correo electronico");
			CORREO= scEspacio.nextLine();
			
			System.out.println("Ingrese la direccion de la cita");
			DIRECCION = scEspacio.nextLine();
			
			System.out.println("Ingrese la fecha de la cita");
			FECHA = scEspacio.nextLine();
			
			System.out.println("Ingrese la hora de la cita");
			HORA = scEspacio.nextLine();
			
			System.out.println("Ingrese una descripcion acerca de la cita");
			DESCRIPCION = scEspacio.nextLine();
			
			sizeRegist = NOMBRE.length()+sepEspacio.length()+
					     TELEFONO.length()+sepEspacio.length()+
					     CORREO.length()+sepEspacio.length()+
					     DIRECCION.length()+sepEspacio.length()+
					     FECHA.length()+sepEspacio.length()+
					     HORA.length()+sepEspacio.length()+
					     DESCRIPCION.length()+sepEspacio.length();
			
			bf.write(sizeRegist+sepEspacio+NOMBRE+sepEspacio+TELEFONO+sepEspacio
					+CORREO+sepEspacio+DIRECCION+sepEspacio+FECHA+sepEspacio+HORA+sepEspacio+DESCRIPCION);
			bf.close();
	...
	public static void mostrarRegistros() {
		try {
		 ...
		 while ((linea= leerfichero.readLine()) !=null) {
			StringTokenizer mitoken = new StringTokenizer(linea, "_");
	...
	public static void eliminarRegistroTelefono() {
		...
		try {
			...
			while(br.ready()){
				...
				StringTokenizer mistokens = new StringTokenizer(linea, "_");
